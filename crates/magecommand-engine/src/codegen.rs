...
/// A generated class's path under `generated/code`, mirroring
...
/// ```
///
/// **Underscores become directories too** — PSR-0, which is what Magento's
/// generated-code autoloader still expects. Splitting only on the namespace
/// separator put `Wyomind\...\Renderer\Configurable_2_1\Interceptor` at
/// `Renderer/Configurable_2_1/Interceptor.php` while Magento writes
/// `Renderer/Configurable/2/1/Interceptor.php`; the autoloader looks for the
/// latter, so the interceptor silently failed to load at runtime. Only the path
/// is rewritten — the class name inside the file keeps its underscores.
fn generated_rel_path(class: &str) -> String {
    class.replace(['\\', '_'], "/")
}
...
pub fn generate_code(
    ...
    for (name, kind) in &cg.emitted {
        ...
        files.push((format!("{}.php", generated_rel_path(name)), content));
    }
    ...
    let interceptors: Vec<Result<(String, String), String>> = plan
        ...
        .map(|(class, methods)| {
            ...
            match bytes {
                Some(bytes) => Ok((format!("{}/Interceptor.php", generated_rel_path(class)), bytes)),
...
mod tests {
    ...
    #[test]
    fn generated_paths_split_underscores_like_magento_io() {
        // Magento\Framework\Code\Generator\Io::generateResultFileName is
        //   str_replace(['\\', '_'], '/', $className) . '.php'
        // Underscores are directory separators too (PSR-0), and the generated-code
        // autoloader looks for exactly that path. Splitting only on the namespace
        // separator left the interceptor at a path nothing would load.
        assert_eq!(
            generated_rel_path("Wyomind\\ElasticsearchCore\\Block\\Product\\Renderer\\Configurable_2_1"),
            "Wyomind/ElasticsearchCore/Block/Product/Renderer/Configurable/2/1"
        );
        // The common case is unaffected.
        assert_eq!(generated_rel_path("Magento\\Catalog\\Model\\Product"), "Magento/Catalog/Model/Product");
        // A leading separator never yields an empty first segment.
        assert_eq!(generated_rel_path("Foo\\Bar_Baz"), "Foo/Bar/Baz");
    }
