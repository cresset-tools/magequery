...
pub fn compute_outputs(magento: &Magento, defs: &mut Definitions, root: &Path) -> CompileOutputs {
    ...
    let mut code = codegen::generate_code(magento, defs, root.to_path_buf(), &area_files, &interception);
    ...
    for _round in 0..8 {
        ...
        code = codegen::generate_code(magento, defs, root.to_path_buf(), &area_files, &interception);
